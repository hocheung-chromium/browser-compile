...
TEST_F(TabStripModelTest, CommandCloseOtherTabs) {
  ...
  EXPECT_EQ("0p 1 2", GetTabStripStateString(tabstrip));
  tabstrip.CloseAllTabs();
  EXPECT_TRUE(tabstrip.empty());
}
...
// Tests IsContextMenuCommandEnabled and ExecuteContextMenuCommand with
// CommandCloseTabsToLeft.
TEST_F(TabStripModelTest, CommandCloseTabsToLeft) {
  TestTabStripModelDelegate delegate;
  TabStripModel tabstrip(&delegate, profile());
  EXPECT_TRUE(tabstrip.empty());

  // Create three tabs, select last two tabs, CommandCloseTabsToLeft should
  // only be enabled for the first tab.
  ASSERT_NO_FATAL_FAILURE(
      PrepareTabstripForSelectionTest(&tabstrip, 3, 0, "1 2"));
  EXPECT_FALSE(tabstrip.IsContextMenuCommandEnabled(
      0, TabStripModel::CommandCloseTabsToLeft));
  EXPECT_TRUE(tabstrip.IsContextMenuCommandEnabled(
      1, TabStripModel::CommandCloseTabsToLeft));
  EXPECT_TRUE(tabstrip.IsContextMenuCommandEnabled(
      2, TabStripModel::CommandCloseTabsToLeft));
  tabstrip.ExecuteContextMenuCommand(0, TabStripModel::CommandCloseTabsToLeft);
  EXPECT_EQ("0 1 2", GetTabStripStateString(tabstrip));
  tabstrip.CloseAllTabs();
  EXPECT_TRUE(tabstrip.empty());
}
...
TEST_F(TabStripModelTest, GetIndicesClosedByCommand) {
  ...
  EXPECT_EQ("",
            indicesClosedAsString(0, TabStripModel::CommandCloseTabsToLeft));
  EXPECT_EQ("0",
            indicesClosedAsString(1, TabStripModel::CommandCloseTabsToLeft));

  EXPECT_EQ("", indicesClosedAsString(0, TabStripModel::CommandCloseOtherTabs));
  EXPECT_EQ("", indicesClosedAsString(1, TabStripModel::CommandCloseOtherTabs));
  ...
  EXPECT_EQ("",
            indicesClosedAsString(0, TabStripModel::CommandCloseTabsToLeft));
  EXPECT_EQ("",
            indicesClosedAsString(2, TabStripModel::CommandCloseTabsToLeft));

  EXPECT_EQ("", indicesClosedAsString(0, TabStripModel::CommandCloseOtherTabs));
  EXPECT_EQ("", indicesClosedAsString(2, TabStripModel::CommandCloseOtherTabs));
