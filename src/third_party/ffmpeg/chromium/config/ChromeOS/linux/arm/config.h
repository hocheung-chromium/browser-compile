 ...
 * --disable-linux-perf --x86asmexe=nasm --optflags='\"-O2\"' --arch=arm
 * --enable-armv6 --enable-armv6t2 --enable-vfp --enable-thumb
 * --extra-cflags='-march=armv7-a' --enable-cross-compile --target-os=linux
 * --extra-cflags='--target=arm-linux-gnueabihf'
 * --extra-ldflags='--target=arm-linux-gnueabihf'
 * --sysroot=/usr/local/google/home/dalecurtis/code/chrome/src/build/linux/debian_bullseye_armhf-sysroot
 * --extra-cflags='-mtune=cortex-a8' --extra-cflags='-mfloat-abi=hard'
 * --extra-cflags=-O2 --disable-neon --extra-cflags='-mfpu=vfpv3-d16'
 * --enable-pic --cc=clang --cxx=clang++ --ld=clang
 * --extra-ldflags='-fuse-ld=lld' --enable-decoder='aac,h264,hevc'
...
/* #define HAVE_VFP_ARGS 1 -- softfp/hardfp selection is done by the chrome
 * build */
